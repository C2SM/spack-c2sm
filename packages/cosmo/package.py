...
class Cosmo(MakefilePackage):
    ...
    def setup_environment(self, spack_env, run_env):
        ...
        if 'cosmo_target=gpu' in spec:
            mpi_spec = spec['mpicuda'].mpifc
        else:
            mpi_spec = spec['mpi'].mpifc
        if self.compiler.name == 'pgi':
            spack_env.set('F90', mpi_spec + ' -D__PGI_FORTRAN__')
            spack_env.set('LD', mpi_spec + ' -D__PGI_FORTRAN__')
        else:
            spack_env.set('F90', MPISPEC)
            spack_env.set('LD', MPISPEC)
