...
class FlexpartIfs(MakefilePackage):
    ...
    depends_on('fdb-fortran@archive_retreive',when='@fdb', type=('build', 'link'))
    depends_on('fdb@5.10.8 ^eckit@1.20.2: ~mpi ^eccodes@2.19.0 +build_shared_libs ^metkit@1.9.2', when='@fdb', type=('build', 'link'))
    build_directory = 'src'
    ...
    def setup_build_environment(self, env):
        ...
        if self.spec.version not in (Version('fdb')):
            ...
            env.set('JASPER_LD_FLAGS', '-Wl,--no-relax' +' -L' + self.spec['fdb'].prefix +
                '/lib'  + ' -lfdb5' + ' -L' + self.spec['fdb-fortran'].prefix + '/lib' + ' -lfdbf')
            # not really required, just a default since the -I flags would be inconsistent with an empty string
            env.set('CURL_INCLUDES', '/usr' 
                + ' -I' + self.spec['fdb-fortran'].prefix + '/include/fortran/fdb/modules'
                + ' -I' + self.spec['fdb'].prefix + '/include')
