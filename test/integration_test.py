...
def spack_info(spec: str, log_filename: str = None):
    ...
    log_with_spack(f'spack info {spec}', 'integration_test',
                         log_filename)
...
def spack_spec(spec: str, log_filename: str = None):
    ...
    log_with_spack(f'spack spec {spec}', 'integration_test',
                         log_filename)
...
@pytest.mark.icon
@pytest.mark.parametrize('variant', ['serialization=create claw=std', 'fcgroup=DACE.externals/dace_icon.-O1', 'extra-config-args=--disable-new_feature,--enable-old_config_arg'])
def test_icon_spec_with_variant(variant: str):
    spack_spec(f'icon {variant}')

@pytest.mark.icon
@pytest.mark.parametrize('wrong_variant', ['fcgroup=dace.externals/dace_icon.-O1', 'extra-config-args=--new_feature'])
@pytest.mark.xfail
def test_icon_spec_with_wrong_variant(wrong_variant: str):
    spack_spec(f'icon {wrong_variant}')

@pytest.mark.cosmo
@pytest.mark.parametrize('variant', ['cosmo_target=gpu ~cppdycore', 'cosmo_target=cpu ~cppdycore'])
