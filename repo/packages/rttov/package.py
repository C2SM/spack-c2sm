...
class Rttov(Package):
    ...
    def url_for_version(self, version):
        return 'file://{0}/rttov{1}.tar.{2}'.format(
            os.getcwd(), version.joined,
            'gz' if int(str(version.up_to(1))) <= 12 else 'xz')
    ...
    def install(self, spec, prefix):
        ...
        if self.compiler.name == 'gcc' or self._compiler_is_mixed_gfortran():
            ...
            if '+openmp' in self.spec:
                for f in ['rttov_mfasis_ad', 'rttov_mfasis_k']:
                    makefile_vars['FFLAGS_ARCH_{0}'.format(f)].append(
                        '-O2 -fp-model source')
        ...
        if '+netcdf' in self.spec:
            ...
            makefile_vars['LDFLAGS_EXTERN'].append(
                self.spec['netcdf-fortran'].libs.link_flags)

        if '+hdf5' in self.spec:
            makefile_vars['FFLAGS_EXTERN'].append('-D_RTTOV_HDF')
            makefile_vars['LDFLAGS_EXTERN'].append(
                self.spec['hdf5:fortran,hl'].libs.link_flags)
